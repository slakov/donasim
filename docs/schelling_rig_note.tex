...
$$
    \frac{y}{x+y}\geq \tau_R.
$$
The one-parameter threshold model is recovered on the diagonal $\tau_B=\tau_R=\tau$. In the finite simulations accompanying this note we use the following discrete update rule. At each step, an unhappy membership $(v,\mathcal{C})$ is considered in random order. The vertex $v$ is allowed to move only to a group $\mathcal{C}'$ to which it does not currently belong and in which its own colour demand would be satisfied after joining. If there is more than one such target, the target is chosen uniformly at random. If no satisfying target exists, including the case in which $v$ already belongs to all $m$ groups, the default simulation rule drops the unhappy membership, so that $v$ leaves $\mathcal{C}$. The dashboard also includes a conservative alternative in which the membership of $v$ in $\mathcal{C}$ is restored and no move is performed; under that convention the simulation may stop in a blocked state. In both cases the simulation stops when all current memberships are happy, and in the conservative convention it also stops when no legal satisfying move exists.

A two-dimensional sweep over $(\tau_B,\tau_R)$ can therefore be used to estimate the segregation surface and to check whether the apparent transition near the symmetric threshold is robust under asymmetric colour demands. More precisely, fixing $n$, $m$, the initial membership probability $p=\lambda/m$, and the initial blue probability $p_B$, for each pair $(\tau_B,\tau_R)$ we run multiple independent simulations and estimate
$$
    \mathbb{E}[S(\tau_B,\tau_R)],
$$
where $S$ is the final fraction of non-empty cliques that are monochromatic. This gives a heat map with coordinates $(\tau_B,\tau_R)$ and colour $\mathbb{E}[S]$. Useful additional diagnostics include the same share restricted to cliques of size at least two so that singleton groups do not artificially inflate segregation, the numbers of blue-only and red-only cliques, colour-specific counts of unhappy individuals, stopping times, and the probability of blocked runs in which no satisfying move is available. As a sanity check, segregation should increase as the two demands increase and should be close to complete when both thresholds are above $1/2$ under the default leave-group rule; in finite simulations with the conservative blocked rule, deviations from this check can occur when no satisfying target group is available.

In the browser implementation, the default heat map uses grid spacing $\Delta\tau=0.10$ and 100 simulations per grid point. Finer options such as $\Delta\tau=0.05$ and $\Delta\tau=0.01$ are also available for producing smoother numerical surfaces, at the cost of increasing the number of grid points from 121 to 441 and 10201 respectively. For final figures, the number of simulations per grid point can be increased up to 1000, and larger values of $n$ and $m$ can be used to reduce finite-size artefacts while keeping the expected group size $n\lambda/m$ in the desired range.
