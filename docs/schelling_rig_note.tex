...
In the browser implementation, the default heat map uses grid spacing $\Delta\tau=0.10$ and 100 simulations per grid point. A finer option $\Delta\tau=0.05$ is also available for producing smoother numerical surfaces, at the cost of increasing the number of grid points from 121 to 441. For final figures, the number of simulations per grid point can be increased up to 1000, and larger values of $n$ and $m$ can be used to reduce finite-size artefacts while keeping the expected group size $n\lambda/m$ in the desired range.
